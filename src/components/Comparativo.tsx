      ...
      <table className="min-w-full w-full border-collapse">
        <thead className="bg-accent-600 text-white sticky top-0 dark:bg-accent-700">
          <tr>
            ...
            {chapas.map(c=> <th key={c.id} className="p-3 border text-left"><a href={`/chapa/${c.id}`}>{c.partidoPrincipal} <span className="text-sm text-accent-100">({c.numeroUrna})</span></a></th>)}
          </tr>
        </thead>
        <tbody>
          {temas.map(t=> (
            <tr key={t.id} className="align-top even:bg-slate-50 dark:even:bg-slate-900">
              <th className="p-3 border align-top text-left align-top">{t.nome}</th>
              {chapas.map(c=>{
